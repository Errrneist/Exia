...
function indent(event) {
	...
	for (var i = 0; i < lines.length; i++) {
		...
		var lineForMatching = line;
		lineForMatching = lineForMatching.replace(/\\"/, "");                       // strip \"
		lineForMatching = lineForMatching.replace(/["]([^\\"]*(\\*)?)+["]/, "");   // strip string literals (might contain {})
		lineForMatching = lineForMatching.replace(/\/\/.*/g, "");                   // strip // comments
